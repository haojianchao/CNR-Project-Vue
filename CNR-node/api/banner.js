...
module.exports = {
	getBannerJewelry : (req, res, next) => {
	    async.waterfall( [
            ...
            ( db, cb ) => {
	            db.collection( 'banner' ).find( {bannerId : eval( '/jewelry/' )}, {} ).toArray( ( err, res ) => {
	                if ( err ) throw err;
	                cb( null, res );
	                db.close();
                })
	...
	getBannerWatch: (req, res, next) => {
	    async.waterfall( [
            ...
            ( db, cb ) => {
	            db.collection( 'banner' ).find( {bannerId : eval( '/watch/' )}, {} ).toArray( ( err, res ) => {
	                if ( err ) throw err;
	                cb( null, res );
	                db.close();
                })
	...
	getLogo: (req, res, next) => {
	    async.waterfall( [
            ...
            ( db, cb ) => {
	            db.collection( 'banner' ).find( {bannerId : eval( '/logo/' )}, {} ).toArray( ( err, res ) => {
	                if ( err ) throw err;
	                cb( null, res );
	                db.close();
                })
